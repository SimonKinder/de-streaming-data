...
class TestGetArticles:
    ...
    @respx.mock
    @pytest.mark.it("Confirm from-date parameter is removed when from_date is None")
    ...
    @respx.mock
    @pytest.mark.it("Confirm the correct data is returned for a succesful request")
    def test_correct_data(self):
        results = {"response": {"results": [1, 2, 3],
                                "total": 3}}
        mock_response = httpx.Response(
            status_code=200,
            request=httpx.Request(method="GET", url="https://test.com"),
